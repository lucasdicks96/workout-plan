...
// Importiere deine neuen Schemas und Typen
import {
...
router.post(
  ...
  authenticatedHandler(
    async (
      req: AuthenticatedRequest<any, any, CreateExerciseBody>, // Typ aus Zod!
      res: Response<ApiSuccessResponse<Exercise>>,
    ) => {
      // ZOD MAGIC: Validiert und extrahiert die Daten.
...
router.put(
  ...
  authenticatedHandler(
    async (
      req: AuthenticatedRequest<any, any, UpdateExerciseBody>, // Typ aus Zod!
      res: Response<ApiSuccessResponse>,
    ) => {
      // Validierung des Bodys
...
router.put(
  ...
  authenticatedHandler(
    async (
      ...
      // 1. Validiere die URL-Parameter (wandelt "123" direkt in number 123 um!)
      const { id } = exerciseIdParamSchema.parse(req.params);

      // 2. Validiere den Body
      const { title, description, categories } = createExerciseBodySchema.parse(
        req.body,
      );

      const result = await exerciseService.putUserExercise(
        id, // Ist jetzt garantiert eine Zahl!
        title,
        description,
        req.user.id,
        categories,
